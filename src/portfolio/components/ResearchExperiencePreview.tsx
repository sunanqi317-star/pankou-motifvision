...
function ResearchExperienceImage({ src, alt }: { src: string; alt: string }) {
  ...
  return (
    <div className="research-experience-media">
      <img
        src={src}
        alt={alt}
        className="research-experience-image"
        loading="lazy"
...
export function ResearchExperiencePreview({ onViewAll }: ResearchExperiencePreviewProps) {
  return (
    <ContentSection id="research-experience" title="Research Experience">
      <div className="content-card-stack research-experience-stack">
        {researchExperience.map((entry) => (
          <article
            key={entry.id}
            className={`content-card ${cardClassName(entry.cardVariant)}`}
          >
